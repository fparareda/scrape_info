...
while (true) {
  pass++;
  const { data, error } = await c.rpc("backfill_denue_mx_bulk_country_chunk", { chunk_size: chunk });
  if (error) {
    errs++;
  ...
  total += n;
  if (n === 0) break;
  if (pass % 10 === 0) console.log(`pass ${pass}: total=${total.toLocaleString()} chunk=${chunk} ${((Date.now()-t0)/1000).toFixed(0)}s`);
}
