...
async function main(): Promise<void> {
  ...
  if (
    ...
    console.warn(
      "[scraper] no sources enabled — set one of: " +
        ...
        "PROLIO_RUN_CDSS_SK_DENTISTS=true",
    );
    return;
  }
  ...
  // 2026-05-19 scout: CA
  if (cdssSkDentistsOn) {
    await withScrapeRun("cdss-sk-dentists", async () => {
      const res = await runCdssSkDentists();
      total += res.inserted + res.updated;
      return {
        rowsFetched: res.fetched,
        rowsUpserted: res.inserted + res.updated,
      ...
      console.error(`[scraper] cdss-sk-dentists crashed:`, (e as Error).message),
    );
  }
