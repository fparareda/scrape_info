...
async function main(): Promise<void> {
  ...
  if (
    ...
    console.warn(
      "[scraper] no sources enabled — set one of: " +
        ...
        "PROLIO_RUN_ICOMEM_MEDICOS=true",
    );
    return;
  }
  ...
  // 2026-05-19 scout: ES
  if (icomemMedicosEsOn) {
    await withScrapeRun("icomem-medicos-es", async () => {
      const res = await runIcomemMedicosEs();
      total += res.inserted + res.updated;
      return {
        rowsFetched: res.fetched,
        rowsUpserted: res.inserted + res.updated,
      ...
      console.error(`[scraper] icomem-medicos-es crashed:`, (e as Error).message),
    );
  }
