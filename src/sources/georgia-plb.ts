 ...
 *
 * Bulk CSV at sos.ga.gov of active licensees across 40+ boards.
 * Override `PROLIO_GEORGIA_PLB_CSV`. `PROLIO_RUN_GEORGIA_PLB=true`.
 */

const DEFAULT_URL =
  "https://sos.ga.gov/sites/default/files/data/active_licensees.csv";
...
async function fetchAll(limit: number): Promise<ScrapedProfessional[]> {
  const url = process.env.PROLIO_GEORGIA_PLB_CSV || DEFAULT_URL;
  ...
  try {
    ...
    console.error(`[georgia-plb] network error: ${(error as Error).message}`);
    return [];
  }
  if (!response.ok) {
    console.error(`[georgia-plb] ${response.status} on ${url}`);
    return [];
  }
  ...
  for (const row of rows) {
    ...
    if (!licence) continue;
    const board = pick(row, ["board", "license_type", "type", "profession"]);
    ...
    const stateRaw = pick(row, ["state"]) || "GA";
    const address = [street, city, stateRaw, zip].filter(Boolean).join(", ");

    out.push(
      normalise({
        source: "georgia-plb",
        sourceId: `georgia-plb:${licence}:${category}`,
        name,
  ...
  console.log(`[georgia-plb] parsed=${out.length}`);
  return out;
}

export const georgiaPlbSource: ScraperSource = {
  name: "georgia-plb",
  enabled() {
    return process.env.PROLIO_RUN_GEORGIA_PLB === "true";
  },
  async fetch() {
    return [];
  },
...
export async function runGeorgiaPlb(): Promise<{
  fetched: number;
  inserted: number;
  updated: number;
  skipped: number;
}> {
  if (!georgiaPlbSource.enabled())
    return { fetched: 0, inserted: 0, updated: 0, skipped: 0 };
  const rawLimit = Number(process.env.PROLIO_GEORGIA_PLB_LIMIT ?? DEFAULT_LIMIT);
  const limit =
  ...
  console.log(
    `[georgia-plb] done — fetched=${records.length} inserted=${inserted} updated=${updated} skipped=${skipped}`,
  );
