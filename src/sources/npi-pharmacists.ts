...
/**
 * NPI Pharmacists — slice of the federal NPI registry filtered to
 * pharmacy taxonomies. Companion to `npi-nurses` (same V2 API pattern).
 *
 * Categoría Prolio: `farmacia`.
 *
 * --- API probe (2026-05-18) -------------------------------------------
 *   GET https://npiregistry.cms.hhs.gov/api/?version=2.1
 *       &taxonomy_description=Pharmacist&state=CA&limit=1
 *     → 200 OK, JSON
 *
 * Sweep 51 states × 4 pharmacy taxonomies × max 1200 rows.
 * Cap via PROLIO_NPI_PHARMACISTS_LIMIT (default 100k).
 *
 * Off by default. `PROLIO_RUN_NPI_PHARMACISTS=true` activates.
 * Monthly cron — see `.github/workflows/scrape-npi-pharmacists.yml`.
 */
...
async function fetchPage(
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[npi-pharmacists] ${state}/${taxonomy}@${skip} → ${res.status}`);
      return null;
    }
    return (await res.json()) as NppesPage;
  } catch (e) {
    console.warn(
      `[npi-pharmacists] ${state}/${taxonomy}@${skip} fetch failed: ${(e as Error).message}`,
    );
    return null;
  }
}

export const npiPharmacistsSource: ScraperSource = {
  name: "npi-pharmacists",
  enabled() {
    return process.env.PROLIO_RUN_NPI_PHARMACISTS === "true";
  },
  async fetch() {
    return [];
  },
...
export async function runNpiPharmacists(): Promise<{
  fetched: number;
  inserted: number;
  updated: number;
  skipped: number;
}> {
  if (!npiPharmacistsSource.enabled()) {
    return { fetched: 0, inserted: 0, updated: 0, skipped: 0 };
  }

  const rawLimit = Number(
    process.env.PROLIO_NPI_PHARMACISTS_LIMIT ?? DEFAULT_LIMIT,
  );
  const limit =
  ...
  try {
    ...
    console.warn(`[npi-pharmacists] city load failed: ${(e as Error).message}`);
    return { fetched: 0, inserted: 0, updated: 0, skipped: 0 };
  }
  if (cityIndex.size === 0) {
    return { fetched: 0, inserted: 0, updated: 0, skipped: 0 };
  ...
  outer: for (const taxonomy of PHARMACY_TAXONOMIES) {
  ...
  console.log(
    `[npi-pharmacists] done — fetched=${fetched} inserted=${inserted} updated=${updated} skipped=${skipped}`,
  );
