...
/**
 * Castilla y León — Registro Industrial / Empresas Instaladoras.
 *
 * Source: datosabiertos.jcyl.es. Default URL is a documented entry
 * point but **must be verified on first run**. Override at runtime
 * with `PROLIO_CYL_INSTALADORES_CSV`.
 *
 * Auto-detects column headers by substring (cif, razón social,
 * municipio, etc.) so a JCyL header reshuffle doesn't break parsing.
 */
...
export const castillaLeonInstaladores: CcaaSource = {
  name: "castilla-leon-instaladores",
  ccaaCode: "CL",
  ...
  async fetch(): Promise<ScrapedProfessional[]> {
    ...
    if (!response.ok) {
      console.error(`[castilla-leon-instaladores] ${response.status} on ${url}`);
      return [];
    }
    ...
    for (const row of rows) {
      ...
      out.push(
        normalise({
          ...
          metadata: {
            ccaa: "CL",
            registry: "castilla-leon-instaladores",
